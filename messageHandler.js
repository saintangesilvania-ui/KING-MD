...
const lastMessageBySender = new Map(); // anti-spam : dernier timestamp par (groupe+numéro)
...
async function runGroupFilters(sock, m, from, body) {
    ...
    try {
        ...
        return; // pas grave si ça échoue, on n'applique juste pas les filtres cette fois
    }
    ...
    if (!senderIsAdmin && isToggled(from, 'antilink') && LINK_REGEX.test(body)) {
        ...
        return true; // message traité, on n'exécute pas de commande dessus
    }

    // Anti-tag (bloque un message qui mentionne énormément de monde, envoyé par un non-admin)
    ...
    // Anti-spam : plus de 5 messages en moins de 10 secondes → averti
    ...
    // Auto-sticker : convertit automatiquement chaque image envoyée en sticker
...
async function handleMessage(sock, m) {
    ...
    // Morpion : si une partie est active dans cette discussion et que le message est
    // juste un chiffre 1-9, on le traite comme un coup plutôt que de chercher une commande.
    ...
    // Annulation d'une action en attente (ex: .kickall) en tapant "stop"
